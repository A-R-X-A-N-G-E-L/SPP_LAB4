...
app.post("/chatroom", async function(request, response) {
    let decision = await new Promise(async (resolve, reject) => {
        let tokenState = await isTokenValid(request, response, request.cookies.token);
        if (tokenState == _FINE_) {
            resolve(_IS_USER_);
...
async function loadFileToServerAndDoTask(request, response, file, counter) {
    ...
    let remadeString = changeAllOccurences(content, request.body.stringToBeChanged, request.body.stringChanger, parseInt(request.body.numberOfChanges), counter);
...
function changeAllOccurences(stringForWork, whatToChange, changeWithThis, count, fileNumber) {
	let maxMatches = 0;

	if (stringForWork.search(new RegExp(whatToChange)) != -1) {
		maxMatches = stringForWork.match(new RegExp(whatToChange, 'g')).length;
	} else {
        allFilesStatus[fileNumber] = _NOTHING_TO_CHANGE_;
        return;
    }

   	if (count > maxMatches || count == 0) {
		stringForWork = stringForWork.replace(new RegExp(whatToChange, 'g'), changeWithThis);
	} else {
        while (count > 0) {
            stringForWork = stringForWork.replace(new RegExp(whatToChange), changeWithThis);
            count--;
        }
    }
